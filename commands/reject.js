...
exports.run = async (client, message, language, prefix, args) => {
  ...
  client.dbConnection.query('SELECT * FROM suggestions WHERE id = ?', [sID], async (error, result) => {
    ...
    client.dbConnection.query('SELECT channel FROM configurations WHERE id = ?', [message.guild.id], async (err, res) => {
      ...
      await client.dbConnection.query('UPDATE suggestions SET status = ? WHERE id = ?', ['Rejected', sID]);
      ...
      message.channel.send({
        embed: new RichEmbed()
          .setAuthor(language.rejectedSuggestionTitle, client.user.avatarURL)
          .setColor(process.env.EMBED_COLOR)
          .setDescription(language.rejectedSuggestion.replace(/<SuggestionID>/g, sID))
          .setTimestamp()
          .setFooter(process.env.EMBED_FOOTER)
      });
